...
async fn test_success() {
    ...
    state
        ...
        .returning(|limit, offset| {
            let entities: Vec<entity::taxes::Model> = vec![];
            Ok(Page {
                limit,
                offset,
    ...
    let req = test::TestRequest::get().uri("/api/taxes").to_request();
...
async fn test_success_with_limit_and_offset() {
    ...
    state
        ...
        .expect_list()
        .once()
        .with(predicate::eq(10), predicate::eq(30))
        .returning(|limit, offset| {
            let entities: Vec<entity::taxes::Model> = vec![];
            Ok(Page {
                limit,
    ...
    let req = test::TestRequest::get()
        .uri("/api/taxes?limit=10&offset=30")
        .to_request();
